...
public class EntregaDAO {  
    public boolean Salvar(Entrega ent)//salva venda
    {
        ...
        try{
            ...
            s.save(ent);//salva o venda no banco
    ...
    public boolean Alterar(Entrega ent)
    {
        Session s = SessionFactoryUtil.openSession(SessionFactoryUtil.openConnection());
        try{
            s.beginTransaction();
            s.merge(ent);//o comando merge verifica se o registro já existe no banco, se sim ele sobrescreve os dados
    ...
    public void Excluir(int id)
    {
        ...
        try{
            s.beginTransaction();
            Criteria criterio = s.createCriteria(Entrega.class);//cria varial de condição
            ...
            List<Entrega> ent = criterio.list();//lista o venda
            s.delete(ent);//deleta
        }catch(Exception e)
        {
            s.getTransaction().rollback();
    ...
    public static List<Entrega> listartodos()
    {
        Session s = SessionFactoryUtil.openSession(SessionFactoryUtil.openConnection());
        try{
            s.beginTransaction();
            Criteria criterio = s.createCriteria(Entrega.class);
            List<Entrega> retorno = criterio.list();
            return retorno;
        }catch(Exception e){
            s.getTransaction().rollback();
            e.printStackTrace();
    ...
    public static List<Entrega> listar(int id)
    {
        Session s = SessionFactoryUtil.openSession(SessionFactoryUtil.openConnection());
        try{
            s.beginTransaction();
